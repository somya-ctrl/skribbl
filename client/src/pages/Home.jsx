...
export default function Home({ onJoin, onCreate }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#111118] p-4 sm:p-6">
      <div className="w-full max-w-sm bg-[#1a1a27] rounded-2xl border border-white/[0.07] p-6 sm:p-9">

        {/* Logo */}
        <div className="flex items-center justify-center gap-2.5 mb-1">
          <div className="w-9 h-9 sm:w-10 sm:h-10 bg-[#6c63ff] rounded-xl flex items-center justify-center">
            <span className="text-white text-base sm:text-lg">✏️</span>
          </div>
          <span className="text-white text-xl sm:text-2xl font-medium tracking-tight">Skribbl</span>
        </div>
        <p className="text-center text-white/30 text-xs tracking-widest mb-5 sm:mb-6">
          draw · guess · win
        </p>
        ...
        <div className="flex bg-[#111118] rounded-xl p-0.5 gap-0.5 mb-4 sm:mb-5">
          {TABS.map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`flex-1 py-2 rounded-[10px] text-[13px] font-medium capitalize transition-all cursor-pointer ${
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <select
                    ...
                    className="w-full bg-[#111118] border border-white/[0.08] rounded-xl px-2 sm:px-3 py-2 text-white text-[13px] outline-none appearance-none cursor-pointer"
                  >
                    {opts.map((o) => (
  ...
  return (
    <button
      ...
      className="mt-1 w-full bg-[#6c63ff] hover:bg-[#5a52e0] active:scale-[0.98] text-white
        py-2.5 sm:py-3 rounded-xl text-[15px] font-medium transition-all cursor-pointer"
    >
      {children}
    </button>
  );
